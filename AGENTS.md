...
- A display-only docker-compose is generated by `buildDockerCompose()` in `electron/docker.ts` and shown in the UI via `sandobox:generate:compose` IPC. It is never consumed by the app — only for user reference to reproduce the sandbox manually.
