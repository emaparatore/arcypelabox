...
- A display-only `docker-compose.yml` is generated by `buildDockerCompose()` in `electron/docker.ts` and shown in the detail page. It is never consumed programmatically — only as a reference for users who want to reproduce the sandbox outside the app.
