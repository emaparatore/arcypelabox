...
export function buildDockerCompose(sandboxId: string): string {
  ...
  const containerName = sanitizeContainerName(name)
  ...
  const networkName = `${createGroupName(name)}-net`
  ...
  if (services.includes("postgres")) {
    ...
    lines.push(`    container_name: ${containerName}-postgres`)
    lines.push(`    deploy:`)
    lines.push(`      resources:`)
    lines.push(`        limits:`)
    lines.push(`          cpus: "1"`)
  ...
  if (services.includes("redis")) {
    ...
    lines.push(`    container_name: ${containerName}-redis`)
    lines.push(`    deploy:`)
    lines.push(`      resources:`)
    lines.push(`        limits:`)
    lines.push(`          cpus: "1"`)
