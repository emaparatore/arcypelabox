...
export function SandboxDetail({ sandbox, sandboxId, onRefresh, onDeleted, onEdit }: Props) {
  ...
  function generateCompose(): string {
    ...
    const lines: string[] = []
    const name = fullRecord.name || "sandbox"
    ...
    lines.push(`# 2. Build the image: docker build -t ${fullRecord.image_tag} .`)
    ...
    // Build OPENCODE_CONFIG JSON matching buildOpenCodeConfig() in docker.ts
    const configObj: Record<string, unknown> = {}
    if (fullRecord.permissions && Object.keys(fullRecord.permissions).length > 0) {
      const permEntries = Object.entries(fullRecord.permissions).map(([key, value]) => {
        const action = value === "allow" ? "allow" : value === "deny" ? "deny" : "ask"
        return `"${key}":"${action}"`
      })
      configObj.permission = JSON.parse(`{${permEntries.join(",")}}`)
    }
    if (fullRecord.providers && fullRecord.providers.length > 0) {
      configObj.provider = Object.fromEntries(
        fullRecord.providers.map((p) => [p.id, { options: {} }]),
      )
    }
